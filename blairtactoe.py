...
pink = (255,105,180)
fuchsia = (255,0,255)
colors = [purple, black, red, blue, green, yellow, orange, pink, fuchsia, white]
dis_width = 800
dis_height = 800
...
def startscreen(xc, cc):
    ...
    brookimg = pygame.transform.scale(brookimg, (so*10, so*10))
    i = -1
    ...
    def circle(cc):
        pygame.draw.circle(dis, cc, [dis_width*3/4, dis_height*1/3], rad/9, int(so))
        pygame.display.update()

    def ex(xc):
        pygame.draw.line(dis, xc, [dis_width*1/6, dis_height*1/4], [dis_width*1/3, dis_height*7/16], int(so))
        pygame.draw.line(dis, xc, [dis_width*1/6, dis_height*7/16], [dis_width*1/3, dis_height*1/4], int(so))
        pygame.display.update()
    ...
    def blank(x, y):
        pygame.draw.rect(dis, bgcolor, [dis_width*x, dis_height*y, dis_width*2/8, dis_height*2.4/8])
        pygame.display.update()
    if xc == black:
        blank(1/8, 1/5)
        blair(1/8, 1/5)
    ...
    "background_button": pygame_gui.elements.UIButton(relative_rect=pygame.Rect((dis_width*1/2-so*5, dis_height*7/8), (so*10, so*2)), text="Background", manager=manager),
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame_gui.UI_BUTTON_PRESSED:
                if event.ui_element == box["start_button"]:
                    gameloop(cc, xc, bgcolor)
            if event.type == pygame_gui.UI_BUTTON_PRESSED:
                if event.ui_element == box["background_button"]:
                    i+=1
                    if i == 10:
                        i = 0
                    bgcolor = colors[i]
            ...
            if event.type == pygame_gui.UI_BUTTON_PRESSED:
                if event.ui_element == box["white_buttono"]:
                    ...
                    circle(cc)
            if event.type == pygame_gui.UI_BUTTON_PRESSED:
                if event.ui_element == box["brook_buttono"]:
                    cc = black
                    blank(5/8, 1/5)
                    brook(5/8, 1/5)
...
def gameloop(cc, xc, bgcolor):
    ...
    brookimg = pygame.image.load("brook.png")
    brookimg = pygame.transform.scale(brookimg, (rad/8, rad/8))
    ...
    def circle(x, y):
        pygame.draw.circle(dis, cc, [dis_width*x, dis_height*y], rad/18, int(so))
        pygame.display.update()

    def ex(x, y, a, b):
        pygame.draw.line(dis, xc, [dis_width*x, dis_height*a], [dis_width*y, dis_height*b], int(so))
        pygame.draw.line(dis, xc, [dis_width*x, dis_height*b], [dis_width*y, dis_height*a], int(so))
        board()
        pygame.display.update()
    ...
    def blank(x, y):
        pygame.draw.rect(dis, bgcolor, [dis_width*x, dis_height*y, dis_width*1/8, dis_height*1/8])
        board()
        pygame.display.update()
